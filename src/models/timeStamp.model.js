...
class TimeStamp {
    ...
    static async getSingle(id) {
        const sql = `SELECT *, DATE_FORMAT(stamp_date, "%Y-%m-%d") AS stamp_date FROM time_stamp WHERE stamp_id  = "${id}"`;
        const [rows] = await pool.execute(sql);
        return rows;
    }
    //update
    ...
    static async getTimeStampOverView() {
        const sql = `SELECT * FROM time_stamp JOIN company ON time_stamp.company_id = company.company_id JOIN staff ON time_stamp.staff_id = staff.staff_id JOIN timestamp_type ON time_stamp.type_id = timestamp_type.type_id`
        const [rows] = await pool.execute(sql);
        return rows;
    }
    ...
    static async getFilterByYearMonth(year, month) {
        const sql = 'SELECT * FROM time_stamp WHERE MONTH(time_stamp.stamp_date) = ? AND YEAR(time_stamp.stamp_date) = ?';
    ...
    static async getFilterBy_User_Company_type(user_id, company_id, type_id, from_yearMonth, to_yearMonth, from_date, to_date, stamp_id) {
        ...
        if (from_yearMonth && to_yearMonth) {
            ...
            sql = sql + ` AND DATE_FORMAT(ts.stamp_date, '%Y-%m') BETWEEN '${fromYear}-${fromMonth.padStart(2, '0')}' AND '${toYear}-${toMonth.padStart(2, '0')}'`;
            hasFilter = true;
        }

...
const handleResetPassword = async (req, res, checkUser, connectionName) => {
    // if user exists in staff table or guest table
    if (checkUser.length) {
        // check if user exists in reset_password table
        const checkIfExistedInResetPassword = await ResetPassword.checkIfExistedInResetTable(
            req.body.email,
            connectionName,
        );
        ...
        if (checkIfExistedInResetPassword.length) {
            ...
            await newResetInformation.updateResetPasswordInformation(req.body.email, connectionName);
...
const forgetPassword = async (req, res) => {
    try {
        ...
        if (req.body.guest) {
            ...
            const checkUser = await ResetPassword.checkIfUserExisted(req.body.email, connectionName);
            await handleResetPassword(req, res, checkUser, connectionName);
        }
    } catch (error) {
    ...
    try {
        ...
        if (ckeckPin.length) {
            ...
            if (isDateTimeInPast(resetPasswordInformation[0]?.expiresAt)) {
                await ResetPassword.deleteUserAfterUpdatePassword(email, connectionName);
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        if (guest) {
            await ResetPassword.updateGuestPassword(resetPasswordInformation[0].email, hashedPassword, connectionName);
        } else {
            await ResetPassword.updatePassword(resetPasswordInformation[0].email, hashedPassword, connectionName);
        }

        await ResetPassword.deleteUserAfterUpdatePassword(resetPasswordInformation[0].email, connectionName);
